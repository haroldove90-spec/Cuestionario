                        ...
                        {clients
                          ...
                          .map((client) => {
                            const clientResponses = responses.filter(
                              (r) =>
                                (client.email && r.contact_email?.toLowerCase() === client.email.toLowerCase()) ||
                                (client.full_name && r.client_name?.toLowerCase() === client.full_name.toLowerCase())
                            );
                            const latestResponse = clientResponses[0];
                            return (
                                    ...
                                    <div className="flex items-center justify-end gap-2">
                                      {clientResponses.length > 1 && (
                                        <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 font-extrabold text-[10px]">
                                          {clientResponses.length} Envíos
                                        </span>
                                      )}
                                      <button
                                        type="button"
                                        onClick={() => handleInspectRecord(latestResponse)}
              ...
              {filteredResponses.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-3">
                  {filteredResponses.map((rec) => {
                    const clientSubmissions = responses.filter(
                      (r) =>
                        (rec.contact_email && r.contact_email?.toLowerCase() === rec.contact_email.toLowerCase()) ||
                        (rec.client_name && r.client_name?.toLowerCase() === rec.client_name.toLowerCase())
                    );
                    const isLatest = clientSubmissions[0]?.id === rec.id;
                      ...
                      >
                        <div className="space-y-1.5 flex-1">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {isLatest && clientSubmissions.length > 1 && (
                              <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-black text-[10px] flex items-center gap-1 border border-emerald-200">
                                <Sparkles className="w-3 h-3 text-emerald-600" /> ÚLTIMA ACTUALIZACIÓN
                              </span>
                            )}
                            {!isLatest && (
                              <span className="px-2 py-0.5 rounded-full bg-slate-200 text-slate-700 font-extrabold text-[10px]">
                                Envío Anterior
                              </span>
                            )}
                            {rec.data?.section3?.attachedFiles && rec.data.section3.attachedFiles.length > 0 && (
                              <span className="px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-900 font-black text-[10px] flex items-center gap-1 border border-emerald-300 shadow-2xs">
                                <Paperclip className="w-3 h-3 text-emerald-700" />
                                {rec.data.section3.attachedFiles.length} ADJUNTO{rec.data.section3.attachedFiles.length > 1 ? 'S' : ''}
                              </span>
                            )}
            ...
            <div id="adminPrintableQuestionnaire" className="p-6 overflow-y-auto space-y-6 text-slate-800 text-xs sm:text-sm">
              ...
              {(() => {
                const clientHistory = responses.filter(
                  (r) =>
                    (selectedRecord.contact_email && r.contact_email?.toLowerCase() === selectedRecord.contact_email.toLowerCase()) ||
                    (selectedRecord.client_name && r.client_name?.toLowerCase() === selectedRecord.client_name.toLowerCase())
                );
                if (clientHistory.length <= 1) return null;
                return (
                  <div className="p-3 bg-blue-50/90 rounded-2xl border border-blue-200/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 text-xs">
                    <div className="flex items-center gap-2">
                      <Layers className="w-4 h-4 text-blue-600 shrink-0" />
                      <div>
                        <span className="font-extrabold text-blue-950 block">Historial de Envíos del Cliente</span>
                        <span className="text-[11px] text-blue-700">
                          Este cliente ha enviado {clientHistory.length} versiones/cuestionarios.
                        </span>
                      </div>
                    </div>
                    <select
                      ...
                      onChange={(e) => {
                        const targetRecord = clientHistory.find((r) => r.id === e.target.value);
                        if (targetRecord) handleInspectRecord(targetRecord);
                      }}
                      className="w-full sm:w-auto px-3 py-1.5 bg-white border border-blue-300 rounded-xl text-xs font-bold text-slate-800 outline-none focus:border-blue-600 shadow-2xs cursor-pointer"
                    >
                      {clientHistory.map((h, idx) => (
                        <option key={h.id} value={h.id}>
                          {idx === 0 ? '★ Última Actualización (' : `Versión #${clientHistory.length - idx} (`}
                          {new Date(h.created_at).toLocaleDateString('es-MX', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}) - {h.status.toUpperCase()}
                        </option>
                      ))}
                    </select>
                  </div>
              ...
              {/* Tarjeta Destacada de Archivos y Documentos Adjuntos del Cliente */}
              {selectedRecord.data?.section3?.attachedFiles && selectedRecord.data.section3.attachedFiles.length > 0 && (
                <div className="p-4 bg-emerald-50/90 rounded-2xl border-2 border-emerald-300 shadow-2xs space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h4 className="font-extrabold text-emerald-950 text-sm">
                          Archivos y Documentos Adjuntos ({selectedRecord.data.section3.attachedFiles.length})
                        </h4>
                        <p className="text-[11px] text-emerald-700 font-medium">
                          Subidos por el cliente y listos para descargar o revisar
                        </p>
                      </div>
                    </div>
                    <span className="text-[10px] font-black px-2.5 py-1 bg-emerald-200/80 text-emerald-900 rounded-full border border-emerald-300">
                      Supabase Storage
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {selectedRecord.data.section3.attachedFiles.map((file) => {
                      ...
                      const isExcel = file.type?.includes('excel') || file.type?.includes('spreadsheet') || /\.(xlsx|xls|csv)$/i.test(file.name);

                      return (
                        <div key={file.id} className="p-3 bg-white rounded-xl border border-emerald-200 shadow-2xs flex flex-col justify-between gap-2.5 hover:border-emerald-400 transition-all">
                          <div className="flex items-start gap-3">
                            {isImage ? (
                              ...
                              <div className={`p-2.5 rounded-xl shrink-0 ${isPdf ? 'bg-rose-100 text-rose-700' : isExcel ? 'bg-emerald-100 text-emerald-800' : 'bg-blue-100 text-blue-700'}`}>
                                {isPdf ? <FileText className="w-5 h-5" /> : isExcel ? <FileSpreadsheet className="w-5 h-5" /> : <FileText className="w-5 h-5" />}
                              </div>
                            )}

                            <div className="min-w-0 flex-1">
                              <p className="font-extrabold text-slate-900 text-xs truncate" title={file.name}>
                                {file.name}
                              </p>
                              <p className="text-[11px] text-slate-500 font-medium">
                                {file.size ? (file.size > 1024 * 1024 ? `${(file.size / (1024 * 1024)).toFixed(2)} MB` : `${Math.round(file.size / 1024)} KB`) : 'Documento adjunto'}
                          ...
                          <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
                            <a
                              ...
                              className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-extrabold transition-all shadow-2xs"
                              title="Abrir o descargar archivo en una nueva pestaña"
                            >
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (navigator.clipboard) {
                                  ...
                                  onSuccessToast('¡Enlace del archivo copiado al portapapeles!');
                                }
                              }}
                              className="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-bold transition-colors shrink-0"
                              title="Copiar enlace directo del archivo"
                            >
                              Copiar Link
                            </button>
                          </div>
